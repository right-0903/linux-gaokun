...
~Dsi panel will land on 7.1.~(landed)
...
Nowadays, this is not fully tested, I am on local repo which is more experimental, and I don't spend too much on public one, you are on your own.
...
| Backlight | works | see [below](#backlight) |
...
| Camera | works | No plan to support tuning data, no plan to improve rear sensor (I just scan the code :P) |
| CPUfreq | works | |
| Display | works | |
...
| RTC | works | |
| Suspend | works | s2idle. To support lid wakeup, EC may stop suspend with a 0xc0 event |
...
| USB-C DisplayPort Alt Mode | works | it may be broken nowadays (PPM init issues) |
...
~In my case, the monitor supports DisplayPort 1.1 with 2.7Gbps per lane, data rate is 4.32Gbps(8b10b coding), basically, it means I can only use 1920x1080@60Hz. However, if we tweak the timings(at your risks), we can support a decent resolution with a rich timings, 24bpp with 2048(+180H.Blanking)x1280(+60V.Blanking)@60hz $\approx$ 4.30Gbps. If the bandwidth limit is exceeded, your bpp will be degraded to 18 (or lower?) to make actual bandwidth under the limit.~ (__outdated__)

If you encounter the following, reboot, it is rare, but still possible. (more frequent ~6.19)
```
...
## Backlight
It is adjusted via DCS. ~It can also be adjusted via I2C, if we can figure out
which i2c bus, we can dim it via I2C.~ (DCS works well now, forget I2C.)
...
Front sensor: hi846@20(2022), s5k4h7@2d(2023, better image quality)

Rear sensor: s5k3l6@10(2022)

This has been checked on only two 2022 models and one 2023 model.

Rear sensor source [here](https://source.puri.sm/Librem5/linux/-/blob/pureos/latest/drivers/media/i2c/s5k3l6xx.c). With modification
```diff
--- s5k3l6xx.c  2026-04-25 19:24:34.921810254 +0800
+++ s5k3l6xx.c  2026-04-25 19:25:27.698067479 +0800
@@ -1592,7 +1592,6 @@
                dev_err(&c->dev,
...
-               goto err_me;
        ...
        state->pixel_clock = (u64)mclk_freq * 80 * 4 / 4 / 6; // extclk * pll_multiplier * 4 / pre_pll_clk_div / vt_pix_clk_div
...
### Color calibration
Factory 3D lut `.cube` can be extracted by this [loader](https://github.com/right-0903/gaokun-color-mgmt). It can be loaded via [https://aur.archlinux.org/packages/vkbasalt] when gaming. To use it with DE, an cube to icc converter is required. (Someone with high performence AI subscription can try to generate it, PR is welcome.)
...
```
        cfg_version display_version: timestamp panel
2023:
...
2022 LTE:
        3e 01: 20 22 09 16 15 12 33 (boe ?)
        3e 02: 20 22 09 16 15 20 59 (csot new ?)
        3e 00: 20 22 09 16 15 03 21 (csot)
```
...
```
Note that every panel supports 3 firmwares. If you have a CSOT panel,
you can flash `43/41 07` or `3e 00`. To do so, simply replace
`himax_thp_drv.dll` (find it online or copy it from a corresponding
model), then reboot. The touchscreen driver will flash the firmware
automatically; during this process, the touchscreen will be
unresponsive. Once it responds again, the flashing is complete.

`43/41 07` and `3e 00` are flashed and tested, on Linux, swiping may
not work well on `43 07` when resuming from suspend.
...
Note that the unit calibration only works with 2.0 firmware which is provided by the downstream, this chip (WCN685x) can be found on sc8280xp qrd / partial sc8380xp crd, so [sc8280xp qrd driver](https://github.com/WOA-Project/Qualcomm-Reference-Drivers/blob/master/8280_QRD/200.0.68.0/qcwlan8280.cab) or [sc8380xp crd driver](https://github.com/WOA-Project/Qualcomm-Reference-Drivers/blob/master/8380_CRD/200.0.57.0/qcwlanhsp8380.cab) is compatible too. And the board file does not seem to be better than the x13s one we mentioned above, the x13s one has a better radio strength. Of course, you can choose to use the x13 one with 2.0 fw.
